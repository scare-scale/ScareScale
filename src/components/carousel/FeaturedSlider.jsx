...
const FeaturedSlider = ({ movies }) => {
  ...
  return (
    <section className="py-6 sm:py-8">
      <div className="relative overflow-hidden w-full" style={{ height: 'clamp(240px, 42vw, 520px)' }}>
        {movies.map((movie, i) => (
          <div
            key={movie.slug}
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 px-4 sm:px-6 pb-5 sm:pb-8 max-w-2xl">
              ...
              <div className="flex items-center gap-2 mb-4">
                <img src="/logo/icon.png" alt="Scare Scale" className="w-6 h-6 flex-shrink-0 drop-shadow-md" draggable={false} />
                {movie.hasReviews ? (
                  <>
                    <span
                      className="text-base font-bold tabular-nums drop-shadow-sm"
                      style={{ color: movie.isApproved ? 'rgb(240 128 128)' : 'rgb(209 213 219)' }}
                    >
                      {movie.overallScore}
                    </span>
                    <span className="text-gray-300 text-sm drop-shadow-sm">{movie.scoreText}</span>
                  </>
                ) : (
                  <span className="text-gray-400 text-base font-bold drop-shadow-sm">—</span>
                )}
              </div>

              <a
                href={`/movie/${movie.slug}`}
                className="inline-block bg-blood-600 hover:bg-blood-700 text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors duration-200"
              >
                View Movie
              </a>
            </div>
          ...
          {movies.map((_, i) => (
            <button
              ...
              style={{
                ...
                backgroundColor: i === current ? 'rgba(255,255,255,0.90)' : 'rgba(255,255,255,0.30)',
              }}
